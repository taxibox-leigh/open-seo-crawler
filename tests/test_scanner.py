...
class FixtureHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        routes = {
            ...
            "/small.webp": (200, "image/webp", b"small", {}),
...
class IntegrationTests(unittest.TestCase):
    def test_complete_resource_scan_and_attribution(self) -> None:
        with FixtureServer() as url:
            result = Scanner(ScannerConfig(max_resource_size=64)).scan(url)
        ...
        self.assertTrue({"resource.http_error", "resource.redirect", "resource.mime_mismatch", "resource.oversized"} <= ids)
    ...
    def test_cli_writes_report_and_meaningful_exit_code(self) -> None:
        with FixtureServer() as url, TemporaryDirectory() as directory:
            ...
            code = main([url, "--output", str(output), "--quiet"])
